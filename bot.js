  ...
  if (
    ...
    if (member.user.bot) {
      ...
      member.guild.channels.cache
        .get(kanalver)
        .send(
          `**${member} Adlı Bot Sunucuya Giriş Yaptı Rol Başarıyla Verildi!**`
        );
    }
});
...
//HgBB Sistem
 ...
 client.on("guildMemberRemove", async member => {
    ...
    var randomMsg = ["**Sunucudan Çıkış Yaptı.** `{sunucu}` **Yineden Gelmeyi Unutmayın** | :outbox_tray:"];
  ...
  client.on("guildMemberAdd", async member => {
    ...
    var randomMsg = ["**Sunucuya Giriş Yaptı.** **`{sunucu}` Hoşgeldin Huzur Verdin.** | :inbox_tray:"];
    var randomMsg_integer =
      randomMsg[Math.floor(Math.random() * randomMsg.length)];
